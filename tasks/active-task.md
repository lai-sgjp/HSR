...
User PIE closeout: 2026-08-09. User confirmed the Relic enhancement PIE happy
path runs without problems. See the 2026-08-09 entry in `PROJECT_STATE.md` for
the blocking stack-cap defect that was fixed to reach this gate.
...
- User Editor/PIE Asset Gate: SATISFIED 2026-08-09 by user PIE confirmation.
- Post-gate fix: dev harness material grant exceeded `MaxStack=99` with a 999
  request, so `AddStack` rejected the whole grant and the player held 0
  material, making every enhancement option unaffordable. Fixed to query the
  registered cap; focused Automation `HSR.UI.RelicEquipment` 4/4.
- Remaining non-blocking follow-up: the Blueprint graph still indexes
  `EnhancementOptions[0]` directly. Bounds-checked `GetEnhancementOption`,
  `GetEnhancementOptionCount` and `HasEnhancementOptions` accessors now exist on
  `UHSRRelicEquipmentWidget`; rewiring the graph needs an Editor session.
