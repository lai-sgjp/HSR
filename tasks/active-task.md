...
- P2-003：仅完成 Coordinator 规划，尚未执行资产作者提交、Teacher 教学或 Coordinator 最终收尾。
- Phase 3：禁止开始。
...
当前唯一下一步不是 Teacher 行动，而是用户完成 Gate 0 资产提交并回传 commit hash。

资产提交后，Coordinator 首次只读核对本卡、归档、实际 commits 与用户资产 commit，写出交接摘要；不得修改 Gameplay。之后 Teacher 首次行动只能读取 `tasks/active-task.md` 及 Coordinator 明确交接材料，先复述教学范围、出题与等待原始作答门禁、作答后才能纠正/评估/提交、允许文档与禁止项，并以以下精确文本结束：
...
- 已完成：P2-002 最终 `PASS` 归档；Coordinator 创建 P2-003 契约。
- 当前门禁：等待用户本人提交 Gate 0 的七个资产并回传 commit hash。
- 未完成：Coordinator 工程交接核对、Teacher 教学/出题、用户原始作答、Teacher 纠正/评估/提交、Reviewer 独立审查/提交、Phase 2 最终 Ready Gate、收尾 commit 与 push 交付。
- Phase 3：不得开始。
