...
Status: `TASK GATE REVISION 1 / RE-REVIEW REQUIRED / USER CONFIRMATION REQUIRED`
...
2. `AHSRGameModeBase` owns production orchestration configuration: Character Catalog, InitialCharacterId and an explicit new-game-default decision. It does not become a Domain authority.
...
4. After Definitions exist, the caller must choose exactly one branch: accepted save restore, or new-game defaults. A successful/existing Profile+Party selection always wins; the default branch cannot run after accepted restore or overwrite a non-empty Party.
...
8. Repeated bootstrap and UI rebuild are idempotent: no duplicate Profile/Party entry, revision, event or delegate binding. Wrong/missing Pawn leaves committed Profile/Party unchanged.
...
- Existing runtime/save: restored selection is preserved; new-game defaults do not execute or overwrite it.
...
- No Character creation/customization, party editing UI, leveling, Equipment read-model aggregation or mutation, inventory mutation or save schema redesign.
...
First Task Gate review=`REVISE`: the allowlist was unfrozen, save/default ordering was ambiguous, Widget hardcoded `Character.A`, Equipment was unsupported scope and tests were vague. Revision 1 closes those items and now requires narrow re-review. After Task Gate PASS, the Implementation role must first restate the frozen card and stop with: `等待用户确认执行 TASK-P17-PATCH-03B。`
