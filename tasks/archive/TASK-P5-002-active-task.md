...
## Coordinator 授权：最小测试入口

允许增加一个仅用于 Development/Editor/PIE 验证的最小测试入口，职责仅限：设置两个 Participant 的测试 Speed、触发一次合法 `ActionResolved`、重复触发、越权触发、失效 Actor 触发，以及 Reset/空队列测试。入口可以是受控的 C++ Developer/Console 测试函数或现有 Battle GameMode 的开发专用调用，但不得创建正式 UI、GameplayAbility、伤害、胜负规则、SaveGame、网络逻辑或 Tick。

允许修改范围仅扩展为：

- `Source/HSR/Battle/HSRTurnManager.h/.cpp`
- `Source/HSR/Battle/HSRBattleCoordinator.h/.cpp`（仅测试接线）
- `Source/HSR/Battle/HSRBattleGameMode.h/.cpp`（仅开发测试入口）
- `tasks/execution-result.md`

测试入口必须可在 Shipping 排除或明确受 `WITH_EDITOR`/Development 条件保护，并记录结构化日志；完成后由执行者提交 commit，再等待用户 PIE 验证。
