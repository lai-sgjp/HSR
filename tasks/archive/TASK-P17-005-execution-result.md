...
## Historical pre-Editor Asset Gate (superseded)

This section records the earlier code-gate handoff and is superseded by the 2026-07-28 user Editor checkpoint above. At that earlier point the six IA assets, `IMC_FrontendNavigation`, `WBP_FrontendShell_P17`, and `WBP_FrontendModuleRoot_P17` did not exist and Editor work was blocked pending `ASSET GATE READY`. They now exist; the remaining evidence gaps are listed below.

## Not verified

- Save All/reopen persistence, full PIE keyboard/mouse/focus matrix and controlled missing-class behavior.
