...
## Phase 4 closeout evidence boundary (2026-07-19)

- 静态 guards、报告级 Build、用户 PIE 观察、`USER ACCEPTED` 和 Independent Reviewer 动态验证是不同证据等级，不能互相替代。
- P4-004 只读核对没有产生 Teacher 原始答案/纠正记录或新的 Independent Reviewer 结论；因此即使三个工作包已 `PASS WITH FOLLOW-UP`，阶段也必须保持 `Not verified`。
- 对 `OpenLevel`/失败恢复、AI 生命周期和单次消费，未来复测应先声明证据来源与触发场景，再记录结论，避免将未触发路径误写为已通过。
...
## 2026-07-17｜Phase 2 GAS 属性、观察链与分层 Debug

### 核心理解

- ASC 是 GAS 运行枢纽，AttributeSet 定义属性和边界规则，GE 描述属性变更，Attribute Delegate 通知已绑定的观察者；Delegate 不是把工作沿管线分派给“执行部位”。
- Gameplay 主链为 `Character/Actor Info → ASC 应用 GE → AttributeSet 最终真源 → Delegate → ViewModel → Widget`。
- 初始 snapshot 是新 HUD 主动读取“此刻的当前值”；Delegate 负责之后发生变化时的增量通知。重建 HUD 只重建观察者并读取当前 snapshot，不重新应用初始化 GE，也不重置属性。
- 初始化入口可因 Re-Possess 再次进入，但初始化 GE 成功应用次数必须保持为 1。Add 重复应用会叠加；GE 的 Override Modifier Op 即使输出合法值，也可能覆盖真实运行状态并重复触发事件，不能用 C++ 虚函数的 `override` 语义理解。
- UI 只是观察者。若 ASC/AttributeSet 真源错误，修改 HUD 显示只能隐藏问题，不能修复其他 Gameplay 消费者读取的状态。
- GE 成功但 HUD 不刷新时，先读取 AttributeSet/ASC 的最终真源；若真源已改变，再按 `Delegate → ViewModel → Widget` 排查。

### 用户学习过程与结论

用户原始作答已经能复述 GAS 数据流骨架、UI 是消费者以及构造期创建对象/运行期建立 Actor Info 的大体原因；原始答案明确表示不了解 snapshot、幂等、Owner/Avatar 与真实 Debug。纠正教学后，用户先掌握了 snapshot 与 Delegate 的区别、UI 非真源，再通过最终填空正确给出 `Delegate → ViewModel → Widget`。

Phase 2 最低学习 Gate 已达到。已掌握项：基本职责和数据流、snapshot/Delegate、UI 非真源、UI 通知链的基本分层排查。非阻断复习项：初始化幂等的精确定义、GE Add/Override Modifier Op 与 C++ `override` 的区别、Owner/Avatar/Actor Info 在不完整生命周期中的影响。这些复习项不等于 Phase 2 工程门禁失败，也不授权进入 Phase 3。

### Reviewer 后续强制补答

Reviewer `REVISE` 后，用户完成了独立补答并保留纠正轨迹：

- Add 初始化 GE 重复应用：最初回答“不变”，经最小纠正后最终回答 `200`。
- HUD 全 0 时先查的真源：最初回答 Actor Info，经纠正后最终回答 `Health`（即 AttributeSet/ASC 最终属性值）。
- Owner 有效但 Avatar 为空：最终确认依赖 Avatar 的行为不可靠（`NO`）。
...
## 2026-07-19｜Phase 4 第一课：四层数据边界与跨关卡生命周期

### 用户原始回答（原样保留）

> dataasset, runtime request, return context, savegame；因为前两个可以跨关卡保存，而actor是关卡中的实例，退出后会销毁；前者销毁后者保存

### 最小纠正

- 顺序和四个名称对应正确：DataAsset、Runtime Request、Return Context、SaveGame。
- DataAsset 是静态资产定义，可以被多个关卡引用，但不是“运行时保存容器”。
- Runtime Request 只有在被 `GameInstanceSubsystem` 托管时，才会跨 `OpenLevel` 暂存；Request 自身是纯值 DTO。
- Return Context 是本次旅行的回程纯值数据，不是永久存档。
- SaveGame 面向跨游戏会话的持久化进度；本阶段 Pending Encounter 不写入 SaveGame。
- 旧 World 中的 Actor 会在 `OpenLevel` 后销毁；不能把 Actor 指针放进 Request/Context。

### 掌握度

- **已掌握：** 四层名称的基本对应关系；Actor 属于关卡实例且切图后销毁。
- **需要复习：** “对象自身能跨关卡保存”与“由 GameInstanceSubsystem 持有的纯值数据能跨 OpenLevel 暂存”不是同一件事；DataAsset、Runtime Request 和 Return Context 的真源/生命周期仍需继续练习。
- **Teacher 状态：** 本课达到基础概念通过，保留上述复习项；不代表 Phase 4 工程 Gate 或 Reviewer 结论已通过。

### 第二课：探索 AI 事件链

Phase 4 的探索敌人不是每帧主动扫描玩家，而是由事件推动状态变化：

```text
AI Perception 发现玩家
        ↓
AIController 更新探索状态（例如 Patrol → Chasing）
        ↓
NavMesh + AI MoveTo 执行追击
        ↓
接触/遇敌条件成立
        ↓
校验 Definition、目标和当前状态
        ↓
RequestEncounter（只允许一次）
        ↓
GameInstanceSubsystem 保存 Pending Request
        ↓
OpenLevel(Map_BattleTest)
```

职责边界：EnemyCharacter 负责角色和表现；AIController 是探索状态、感知回调和移动请求的协调者；AI Perception 提供发现/丢失事件；NavMesh/MoveTo 负责路径移动；TransitionSubsystem 才是跨图 Pending Request 的运行时真源。敌人发现玩家不等于已经提交 Encounter，因为还必须通过 Definition、地图、当前状态和“是否已有 Pending”校验。成功后进入 `EncounterPending`，停止后续移动和重复提交。

当前证据边界必须保留：P4-002 主路径主要有用户 PIE 证据；A1 完整 Build log 已删除，仅保留报告级声明与时间链。目标销毁、重复 perception、MoveTo Failed/Aborted 和独立 UnPossess 专项未完整动态验证。P4-004 当前阶段结论仍为 `Not verified`。

### 第二课基础问题（只回答这两题）

1. AI Perception、AIController、NavMesh/MoveTo、TransitionSubsystem 在上面链条中各自负责哪一件事？
2. 为什么“发现玩家”后不能直接重复调用 `RequestEncounter`？成功提交后应进入什么状态？

### 第二课用户原始回答（原样保留）

> 只知道 TransitionSubsystem 负责地图跳转，其余不知道；第二题不知道。

### 第二课补充教学：从快递流程理解 AI 事件链

可以把一次遭遇想成送快递：

| 生活流程 | HSR 对应 | 作用 |
|---|---|---|
| 摄像头发现门口有人 | AI Perception | 产生“发现/丢失目标”的感知事件，不决定整场遭遇 |
| 司机决定接单、选择是否追上 | AIController | 保存探索状态，收到事件后决定 Patrol/Chasing，并发起移动请求 |
| 道路和导航软件 | NavMesh / AI MoveTo | 根据可行走区域计算并执行从当前位置到目标的移动 |
| 仓库交接和跨区转运 | TransitionSubsystem | 验证并保存纯值 Encounter Request，执行/协调 OpenLevel 后的跨图交接 |

映射到 HSR 代码职责：`AI Perception` 只负责告诉 `AHSREnemyAIController`“看见或失去玩家”；Controller 是探索状态和移动请求的协调者；NavMesh/MoveTo 负责路径执行；`UHSRBattleTransitionSubsystem` 是跨 World 的 Pending Request 真源。Enemy Character 提供敌人 Actor/表现，但不应复制一套 AI 状态，也不应自己成为第二个旅行真源。

“发现玩家”不等于“提交 Encounter”。发现事件后，Controller/统一入口还必须检查：敌人当前是否允许遇敌、Enemy/Encounter Definition 是否有效、Battle Map 是否存在、请求是否已经 Pending、目标是否仍有效。所有校验成功后才调用 `RequestEncounter`；成功后状态进入 `EncounterPending`，停止继续追击/移动，并拒绝重复 Perception、Overlap 或重复请求。失败则保留结构化失败原因，不能假装已经切图。

本轮学习进度：TransitionSubsystem 的跨图职责已被用户识别；Perception、Controller、NavMesh/MoveTo 的分工尚未独立掌握，已用生活类比重新讲解。第二题（发现不等于提交）尚未掌握，等待下一次匹配练习。P4-002 的 Build/运行证据等级和 P4-004 `Not verified` 边界继续不变。
...
1. AI Perception 对应哪一项？
2. AIController 对应哪一项？
...
- **基础职责匹配已掌握：** AI Perception 负责产生发现事件；AIController 负责保存探索状态并决定是否追击。
- NavMesh/AI MoveTo 仍需继续巩固：它只负责把角色沿可行走路径移动到目标附近，不决定是否开始战斗，也不拥有 Encounter 状态。

### 第二课微课：移动与遇敌提交是两件事

生活类比：导航软件可以把司机带到仓库门口，但导航软件不会决定“这批货是否已经正式交接”。

- **NavMesh / AI MoveTo**：解决“怎么走到那里”，执行路径移动。
- **Encounter 规则与 TransitionSubsystem**：解决“是否允许交接、交接什么数据、切图后把数据交给谁”。
- **EncounterPending**：像仓库已经盖章的“交接处理中”锁。一次 `RequestEncounter` 成功后立即进入该状态，停止继续追击或移动，并拒绝重复 Perception、Overlap 和请求。它不是战斗状态机，也不提前代表胜利或失败。

因此，NavMesh/MoveTo 不能直接调用第二套 OpenLevel 逻辑；TransitionSubsystem 也不负责计算路径。两者通过清晰的事件/请求边界协作。

P4-002 主路径、Build 日志缺失和用户证据等级保持原样；P4-004 仍为 `Not verified`，本课只记录学习进度，不改变工程门禁。

### 第三轮基础选择题

1. 谁负责路径移动？
   - A. NavMesh / AI MoveTo
   - B. TransitionSubsystem

2. `RequestEncounter` 成功后进入什么状态？
   - A. Patrol
   - B. EncounterPending
...
- **已掌握：** NavMesh / AI MoveTo 负责路径移动；成功 `RequestEncounter` 后进入 `EncounterPending`。

### 第三课：RequestEncounter 提交前的四项检票

把提交 Encounter 想成出发前检票。即使乘客已经走到车站（AI 已发现玩家），也不能直接发车，必须逐项确认：

1. **目标有效：** 玩家目标仍存在且仍满足遇敌条件；如果目标已销毁或失效，拒绝请求。
2. **Definition 有效：** Enemy/Encounter 定义存在，稳定 ID 能找到合法配置；缺定义不能凭空构建遭遇。
3. **Battle Map 有效：** 目标地图路径存在且可作为旅行目标；无效地图不能先写入半有效 Pending。
4. **没有已有 Pending：** TransitionSubsystem 当前没有另一笔未消费请求；已有请求时拒绝新的请求，避免重复切图和覆盖原始事实。

只有四项都通过，才写入纯值 Request、进入 `EncounterPending` 并开始旅行。任何一项失败都应返回结构化失败原因，不应伪装成成功或覆盖已有请求。

### 第三课：三种 Initiative 的直觉

Initiative 表示“这次遭遇中谁占先”，是遭遇开始时记录的一条事实，不是当前 HP，也不是战斗中每回合的行动顺序：

- **Player Advantage：** 玩家先发现或先接触，玩家占先。
- **Enemy Advantage：** 敌人先发现并主动接近，敌人占先。
- **Neutral：** 没有明确一方占先，采用中性起始条件。

它应被写入 Encounter Request，让 Battle Map 读取这次遭遇已经确定的来源事实；本阶段不实现 Phase 5 的 TurnManager 或详细回合排序。

P4-003 的三种 initiative 有动态日志，但其余失败回调专项仍是用户接受/延期边界；P4-004 阶段状态仍为 `Not verified`。

### 第三课选择题

1. 如果 TransitionSubsystem 已经有 Pending Request，再来一个请求应该怎样？
   - A. 接受并覆盖旧请求
   - B. 拒绝新请求
...
- **已掌握：** 已有 Pending 时拒绝新请求，不覆盖旧请求；Initiative 表示本次遭遇的占先/来源事实，而不是 HP。

### 第四课：失败恢复与 Context 单次消费

把 Encounter Request 想成“订单”，把 ReturnContext 想成“回程票”：

- **无效订单不能入库：** 如果 Battle Map 无效、Definition 无效、目标失效或已有 Pending，请求失败，不应把半成品写入 Pending。合法失败后，系统仍应保持可继续探索/重试的状态。
- **已有订单不能覆盖：** TransitionSubsystem 已有 Pending 时，第二份请求必须拒绝，保护第一份订单的 RequestId、EncounterId 和返回信息。
- **订单只取一次：** Encounter Request 在 Battle Map 第一次 Consume 成功后立即清理并标记已消费；第二次 Consume 返回结构化 `AlreadyConsumed`，不能再次得到旧订单。
- **回程票也只验一次：** ReturnContext 保存探索地图路径、Return Transform 和 RequestId 等纯值。返回时应先复制出结果，再清空内部 Context；这样既能恢复正确位置，也能防止第二次消费拿到旧 Transform 或重复返回。

P4-001/003 的源码静态边界支持“复制后清空、单次消费”；P4-003 有多次 Return 二次消费 `AlreadyConsumed` 动态日志。travel failure、AlreadyPending、失败后重试等专项动态证据仍是用户接受/延期边界，P4-004 仍为 `Not verified`。

### 第四课选择题

1. 失败的无效 Map 请求是否应保留脏 Pending？
   - A. 是
   - B. 否

2. ReturnContext 消费成功后，第二次消费应得到什么？
   - A. `AlreadyConsumed`
   - B. 旧 Transform
...
- **已掌握：** 失败请求不留下脏 Pending；ReturnContext 第二次消费应返回 `AlreadyConsumed`，不能返回旧 Transform。

### 第五课：Phase 4 与 Phase 5 的边界

Phase 4 的任务是把“探索中的敌人如何发起一次稳定的跨图遭遇”讲清楚：

- 探索 Enemy Character 和 AIController；
- AI Perception、NavMesh/MoveTo、巡逻/追击状态；
- EncounterDefinition、EnemyDefinition 和稳定 ID；
- 纯值 Encounter Request / ReturnContext；
- GameInstanceSubsystem 的 Pending、单次 Consume 和旅行校验；
- 空 `Map_BattleTest` 的加载、消费和测试返回数据。

Phase 4 不负责真正的战斗规则。以下属于 Phase 5：

- Battle Actor 与 ASC 的重建；
- Battle Participant 和 TurnManager；
- Speed 排序与行动循环；
- GameplayAbility、普攻、伤害 GE；
- 死亡、Victory/Defeat、奖励；
- 正式 BattleResult 和战后返回探索闭环。

### 证据等级

- **S（Static）**：源码、反射、GC、Tick、边界的只读静态检查；说明设计和代码结构，但不证明运行成功。
- **B（Build）**：可定位的 UHT/C++/Link 构建日志；证明某次代码能构建，但不证明 Editor/PIE 行为。
- **D（Dynamic）**：实际 Editor/PIE 日志、截图或运行结果；证明特定运行路径，但要注明执行者和版本时间。
- **User**：用户提供的 Editor/PIE 自述或文件；可以记录为用户证据，不冒充 Reviewer 亲自运行。
- **USER ACCEPTED**：用户明确接受未补齐的风险/证据缺口；这不是动态通过，仍必须在 follow-up 中保留。

本阶段真实状态仍是：P4-001～003 各自 `PASS WITH FOLLOW-UP`，P4-004 因 Teacher/Reviewer 收尾尚未闭合而为 `Not verified`。本课完成的是学习边界，不改变工程门禁。

### 第五课选择题

1. `TurnManager` 属于哪个阶段？
   - A. Phase 4
   - B. Phase 5

2. 用户自报的 PIE 结果属于哪种证据？
   - A. Reviewer 亲自动态证据
   - B. User 用户证据
...
- **已掌握：** `TurnManager` 属于 Phase 5；用户自报的 PIE 结果属于 User 证据，不是 Reviewer 亲自运行的动态证据。

#### 用户提出的工程规则需求

- Phase 收尾前必须保留原始 Build 日志，用于阶段 Gate 的证据核对。
- Phase 收尾后可以保留摘要和关键错误信息，并清理冗余的 `Build-Log-*` 产物；清理不得抹去关键证据、错误原因、时间链或证据等级。
