...
EHSRSaveDecodeResult HSRSaveVersion::DecodeCanonicalPayload(const TArray<uint8>& P,FHSRSaveData& O)
{
	...
	const uint32 PartyCount=R.I<uint32>();if(!R.bOk||PartyCount!=PartySlotCount)return EHSRSaveDecodeResult::NonCanonical;
	...
	if(!ReadCount(R,D.Equipment))return EHSRSaveDecodeResult::InvalidPayload;FGuid PrevGuid;
	for(int32 I=0;I<D.Equipment.Num();++I){auto&X=D.Equipment[I];if(!R.NameRequired(X.DefinitionId))return EHSRSaveDecodeResult::NonCanonical;X.InstanceId=R.Guid();X.CharacterId=R.Guid();X.Kind=R.I<int32>();X.Slot=R.I<int32>();X.EnhancementLevel=R.I<int32>();if(!X.InstanceId.IsValid()||!X.CharacterId.IsValid()||X.Kind<0||X.Kind>1||X.Slot<0||(X.Kind==0&&X.Slot>static_cast<int32>(EHSREquipmentSlot::Feet))||(X.Kind==1&&X.Slot>static_cast<int32>(EHSRRelicSlot::LinkRope)))return EHSRSaveDecodeResult::NonCanonical;const uint32 M=R.I<uint32>();if(!R.bOk||M>MaxCount)return EHSRSaveDecodeResult::InvalidPayload;X.Modifiers.SetNum(M);for(auto&V:X.Modifiers){const uint8 Stat=R.U8();if(Stat>static_cast<uint8>(EHSREquipmentStat::Speed))return EHSRSaveDecodeResult::NonCanonical;V.Stat=static_cast<EHSREquipmentStat>(Stat);double Z;if(!R.D(Z))return EHSRSaveDecodeResult::NonCanonical;V.Value=Z;}if(!R.Name(X.SetId))return EHSRSaveDecodeResult::NonCanonical;X.AuthorityRevision=R.I<int32>();if(I&&!(PrevGuid<X.InstanceId))return EHSRSaveDecodeResult::NonCanonical;PrevGuid=X.InstanceId;}
	...
	D.Map.Revision=R.I<int64>();if(!R.bOk||R.P!=P.Num())return EHSRSaveDecodeResult::InvalidPayload;O=MoveTemp(D);return EHSRSaveDecodeResult::Success;
}
EHSRSaveDecodeResult HSRSaveVersion::MigrateToCurrent(FHSRSaveData& D){if(D.SchemaVersion<1)return EHSRSaveDecodeResult::TooOld;if(D.SchemaVersion>CurrentSchema)return EHSRSaveDecodeResult::FutureSchema;while(D.SchemaVersion<CurrentSchema){switch(D.SchemaVersion){case 1:D.Equipment.Reset();D.Inventory=FHSRInventorySaveData();D.Rewards=FHSRRewardSaveData();++D.SchemaVersion;break;case 2:D.Inventory=FHSRInventorySaveData();D.Rewards=FHSRRewardSaveData();++D.SchemaVersion;break;case 3:D.Quests=FHSRQuestSaveData();++D.SchemaVersion;break;case 4:D.Map=FHSRMapSaveData();++D.SchemaVersion;break;case 5:++D.SchemaVersion;break;default:return EHSRSaveDecodeResult::MigrationFailed;}}if(D.PartyRevision<0||D.Inventory.Revision<0||D.Rewards.Revision<0||D.Quests.Revision<0||D.Map.Revision<0)return EHSRSaveDecodeResult::MigrationFailed;for(const FHSRSaveProfileDto& P:D.Profiles)if(P.RuntimeRevision<0||P.State.Level<1||P.State.Experience<0||P.State.Ascension<0)return EHSRSaveDecodeResult::MigrationFailed;return EHSRSaveDecodeResult::Success;}
