...
bool FHSRBehaviorTreeAdapterPatchTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Fallback controller apply has full zero mutation and explicit unbound runtime"), ControllerApplySnapshot() == ControllerBeforeApply && !RadiusController->PrimaryActorTick.bCanEverTick && !RadiusController->HasRuntimeBlackboardForAutomation());
	...
	TestTrue(TEXT("Normalized controller apply has full zero mutation"), ControllerApplySnapshot() == ControllerBeforeApply);
	...
	TestTrue(TEXT("Negative controller apply has full zero mutation"), ControllerApplySnapshot() == ControllerBeforeApply);
	...
	Controller->PublishPatrolIntentForAutomation(nullptr, ExpectedSpawnOrigin, CandidatePatrolLocation, EHSRPatrolIntentResult::Reachable);
	...
	Controller->PublishPatrolIntentForAutomation(nullptr, ExpectedSpawnOrigin, FVector::ZeroVector, EHSRPatrolIntentResult::ProjectPointFailed);
	TestEqual(TEXT("Projection failure waits instead of issuing a movement loop"), Controller->GetCurrentState(), EHSREnemyExplorationState::PatrolWaiting);
	TestTrue(TEXT("Projection failure publishes SpawnOrigin fallback"), Controller->GetPatrolLocationForAutomation(PatrolLocation));
	TestEqual(TEXT("Projection failure fallback location is SpawnOrigin"), PatrolLocation, ExpectedSpawnOrigin);
	Controller->PublishPatrolIntentForAutomation(nullptr, ExpectedSpawnOrigin, FVector::ZeroVector, EHSRPatrolIntentResult::RandomReachableFailed);
	TestEqual(TEXT("Random failure has the same bounded fallback state"), Controller->GetCurrentState(), EHSREnemyExplorationState::PatrolWaiting);
	TestTrue(TEXT("Random failure publishes SpawnOrigin fallback"), Controller->GetPatrolLocationForAutomation(PatrolLocation));
	TestEqual(TEXT("Random failure fallback location is SpawnOrigin"), PatrolLocation, ExpectedSpawnOrigin);
	TestTrue(TEXT("Projection and random failure reasons remain distinct"), EHSRPatrolIntentResult::ProjectPointFailed != EHSRPatrolIntentResult::RandomReachableFailed);
	...
	Controller->PublishMoveFailureRecoveryForAutomation(TeardownBlackboard, ExpectedSpawnOrigin);
	TestEqual(TEXT("Move failure recovery records MoveFailed before return intent"), Controller->GetLastRecoveryStateForAutomation(), EHSREnemyExplorationState::MoveFailed);
	...
	Controller->ResetRecoveryMarkerForAutomation();
	const int32 MovingEpochBeforeFailure = Controller->GetBehaviorTreeEpoch(); const int32 MovingBBEpochBeforeFailure = TeardownBlackboard->GetValueAsInt(TEXT("TreeEpoch")); const int32 MovingAttemptsBeforeFailure = Controller->GetEncounterSubmissionAttemptsForAutomation();
	...
	TestTrue(TEXT("Moving handled recovery preserves epochs/counts/request/targets"), Controller->GetBehaviorTreeEpoch() == MovingEpochBeforeFailure && TeardownBlackboard->GetValueAsInt(TEXT("TreeEpoch")) == MovingBBEpochBeforeFailure && Controller->GetEncounterSubmissionAttemptsForAutomation() == MovingAttemptsBeforeFailure && TeardownBlackboard->GetValueAsName(TEXT("EncounterRequestId")).IsNone() && Controller->GetCurrentTargetForAutomation() == nullptr && TeardownBlackboard->GetValueAsObject(TEXT("TargetActor")) == nullptr);
	TestTrue(TEXT("MovingToPatrol handled recovery fully publishes return Blackboard intent"), Controller->GetCurrentState() == EHSREnemyExplorationState::ReturningToSpawnOrigin && TeardownBlackboard->GetValueAsEnum(TEXT("AIState")) == static_cast<uint8>(EHSREnemyExplorationState::ReturningToSpawnOrigin) && TeardownBlackboard->GetValueAsObject(TEXT("TargetActor")) == nullptr && TeardownBlackboard->GetValueAsVector(TEXT("SpawnOrigin")) == ExpectedSpawnOrigin && TeardownBlackboard->GetValueAsVector(TEXT("PatrolLocation")) == ExpectedSpawnOrigin && !Controller->HasActiveEncounterRequestForAutomation() && !Controller->IsNavReadyRetryScheduledForAutomation());
	Controller->SetStateForAutomation(EHSREnemyExplorationState::ReturningToSpawnOrigin);
	Controller->ResetRecoveryMarkerForAutomation();
	const int32 ReturningEpochBeforeFailure = Controller->GetBehaviorTreeEpoch(); const int32 ReturningBBEpochBeforeFailure = TeardownBlackboard->GetValueAsInt(TEXT("TreeEpoch")); const int32 ReturningAttemptsBeforeFailure = Controller->GetEncounterSubmissionAttemptsForAutomation();
	...
	TestEqual(TEXT("Returning failure independently records MoveFailed source"), Controller->GetLastRecoveryStateForAutomation(), EHSREnemyExplorationState::MoveFailed);
	TestTrue(TEXT("Returning handled recovery preserves epochs/counts/request/targets"), Controller->GetBehaviorTreeEpoch() == ReturningEpochBeforeFailure && TeardownBlackboard->GetValueAsInt(TEXT("TreeEpoch")) == ReturningBBEpochBeforeFailure && Controller->GetEncounterSubmissionAttemptsForAutomation() == ReturningAttemptsBeforeFailure && TeardownBlackboard->GetValueAsName(TEXT("EncounterRequestId")).IsNone() && Controller->GetCurrentTargetForAutomation() == nullptr && TeardownBlackboard->GetValueAsObject(TEXT("TargetActor")) == nullptr);
	TestTrue(TEXT("Returning handled recovery preserves full bounded Blackboard intent"), TeardownBlackboard->GetValueAsEnum(TEXT("AIState")) == static_cast<uint8>(EHSREnemyExplorationState::ReturningToSpawnOrigin) && TeardownBlackboard->GetValueAsObject(TEXT("TargetActor")) == nullptr && TeardownBlackboard->GetValueAsVector(TEXT("SpawnOrigin")) == ExpectedSpawnOrigin && TeardownBlackboard->GetValueAsVector(TEXT("PatrolLocation")) == ExpectedSpawnOrigin && !Controller->HasActiveEncounterRequestForAutomation() && !Controller->IsNavReadyRetryScheduledForAutomation());
	...
	TestEqual(TEXT("Destroyed chase target publishes ReturningToSpawnOrigin"), Controller->GetCurrentState(), EHSREnemyExplorationState::ReturningToSpawnOrigin);
	...
	const auto SameCharacterRuntime = [OriginEnemy, CharacterLocationBeforeConfig, CharacterSpawnBeforeConfig]() { return !OriginEnemy->PrimaryActorTick.bCanEverTick && OriginEnemy->GetActorLocation() == CharacterLocationBeforeConfig && OriginEnemy->GetSpawnOrigin() == CharacterSpawnBeforeConfig; };
	OriginEnemy->EnemyDefinition = nullptr;
	OriginEnemy->ApplyDefinitionEncounterConfigForAutomation();
	TestTrue(TEXT("No-Definition Character apply preserves encounter fallback and runtime state"), FMath::IsNearlyEqual(OriginEnemy->GetEncounterRadiusForAutomation(), 200.0f) && SameCharacterRuntime());
	...
	TestTrue(TEXT("Definition Character apply has zero runtime mutation"), SameCharacterRuntime());
	...
	TestTrue(TEXT("Negative Character apply has zero runtime mutation"), SameCharacterRuntime());
