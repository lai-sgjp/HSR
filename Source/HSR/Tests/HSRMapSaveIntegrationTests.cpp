...
bool FHSRMapSaveV5IntegrationTest::RunTest(const FString&)
{
	...
	TestEqual(TEXT("schema is v7"), Captured.SchemaVersion, 7);
	...
	for (int32 LegacyVersion = 1; LegacyVersion <= 4; ++LegacyVersion)
	{
		...
		TestEqual(FString::Printf(TEXT("v%d normalizes schema"), LegacyVersion), F.Save->GetSnapshot().SchemaVersion, 7);
