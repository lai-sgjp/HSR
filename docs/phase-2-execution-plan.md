...
| P2-002 | 证明属性变化、边界和生命周期闭环可靠 | 测试 GE、Delegate/解绑完善、重建与失败路径证据 | Effect 改值后 UI 恰好刷新一次；Clamp、Re-Possess、Widget 重建和缺失 GE 安全 |
| P2-003 | 完成工程、教学与阶段收尾 | 最终回归、Teacher 产物、文档与 Coordinator 收尾 | 工程 Gate、学习 Gate、角色交接和 Git 交付齐全 |
...
1. Coordinator 核对 P2-001/P2-002 的源码、资产作者、执行者与 Reviewer commits、Build、Editor 重开、PIE 主路径和失败路径。
2. Coordinator 将源码、diff、证据、真实错误和未验证风险交接给 Teacher。
3. Teacher 完成概念教学、源码阅读、用户复述、预测题与真实 Debug 练习记录，并创建独立 Teacher commit。
4. Teacher 向 Coordinator 交付教学产物、掌握度、未掌握项和 commit hash。
5. Coordinator 复核工程 Gate、学习 Gate、文档与工作树，创建 Phase 收尾 commit；按项目规则处理 push，不 force push。
...
- Coordinator 完成 Phase 收尾 commit，并如实记录 push 结果。
...
当前唯一下一步是 Implementation Agent 按 `tasks/active-task.md` 只读复述 `TASK-P2-002：属性变化、边界与生命周期专项闭环`，并等待用户单独确认。P2-002 完成 Reviewer 审查与 Coordinator 归档前不得进入 P2-003、Ability、伤害或 Phase 3。
