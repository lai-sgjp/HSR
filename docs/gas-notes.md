...
## Phase 7 Execution 与事务收尾（2026-07-20）

- P7-001～P7-004 已完成统一伤害、确定性 Crit、prepared transaction、SP rollback、Ultimate Cost/Refund、Breakdown 对账和 Editor-only follow-up 矩阵；阶段仍保留 `PASS WITH FOLLOW-UP` 证据边界。
- Damage Execution 只 Capture 当前属性并输出 IncomingDamage；CoreAttributeSet 负责最终扣 Health。`FinalDamage` 是公式结果，`AppliedDamage` 是真实 HP delta，不能由 UI 或日志重新计算替代。
- Coordinator 负责 ActionId、RNG stream-copy、事务提交和 Turn/terminal 授权；Ability 只消费 prepared Spec。Editor-only 注入不能冒充底层 GAS/捕获故障证据。
