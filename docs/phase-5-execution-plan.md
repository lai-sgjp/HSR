...
当前状态：仓库已有 Transition/Encounter、BattleTestConsumer 与基础 ASC/CoreAttributeSet，但尚未发现 BattleCoordinator、BattleParticipant、BattleGameMode、TurnManager、BattleResult 和最小攻击 Ability/GE。PHASE4 的正式收尾 commit/push 记录仍是进入 PHASE5 的前置门禁；没有真实 Build/PIE 证据时，门禁状态为 **Not verified**，不能把计划当作完成证据。
...
产出：fresh Development Editor Build/UHT/Link 记录，Editor 重开与连续 PIE 证据，正常/失败矩阵，日志/截图，learning journal、interview notes、worklog、todo、README 与本计划更新。Reviewer 只读审查，Teacher 复盘，Coordinator 更新 PROJECT_STATE 并创建阶段收尾 commit/push。只有 Reviewer PASS 或用户明确接受风险后，PHASE5 才可标记 Ready。
