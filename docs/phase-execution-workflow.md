...
- 每个任务仍只允许一个可独立验收结果，但默认选择能产生用户可见运行结果的最小完整垂直切片/工作包；不得按类、单文件或单资产机械拆卡。
- 一个 Phase 通常规划 2～5 个工程工作包，再加 1 个阶段收尾工作包；按真实复杂度调整，不为满足数量硬拆或硬并。
- 同一工作包可以包含紧密关联的 C++、Editor 配置、PIE 与文档证据，但不同人格和资产作者仍须独立交接并提交自己的实际产物。
- 只有出现不同作者/权限、危险 Git 或 Config、第三方资产、新模块、外部依赖、独立失败/回滚边界，或范围明显超出低级模型可安全处理能力时才拆分。
...
- 每个角色完成本轮任务后，提交自己的实际产物；没有文件变化时先写入对应结果文件，不创建空提交。
- commit message 固定为 `角色+人格+当前任务与阶段+简短告诉自己做了什么`。
- 提交前检查 status、diff、任务允许范围和派生产物；记录 commit hash。
- 一个 Phase 的全部子任务完成、审查、归档并完成角色提交后，由高级协调者创建阶段收尾 commit 并推送远端。
- Phase 中途不 push，不强制 push；失败时保留第一处真实错误。
...
3. **唯一下一任务**：只推荐一个最小连贯垂直切片（smallest coherent vertical slice），以单一可验收的用户可见结果为边界。
