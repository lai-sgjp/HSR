...
- [ ] 完整验证 Visual Studio、UBT/UHT、Development Editor 构建和实际 C++ 标准。（构建、实际编译/链接和运行门禁已有证据；实际 C++ 标准仍未验证。）
- [ ] 验证 Enhanced Input 的默认移动与镜头。
- [ ] 开始 GAS 和回合制最小闭环。

当前仓库已完成 Phase 0 工程、插件/模块、八个根 Gameplay Tags、默认 `Map_ProjectSetup`、Editor 重开和空白 PIE 基线。Phase 0 仍为 `Not verified`，唯一技术证据缺口是实际 C++ 标准；尚未开始 Phase 1 Gameplay。
...
- C++（标准跟随 UE5.6/UnrealBuildTool 实际默认设置）
- Enhanced Input
